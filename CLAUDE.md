...
> 데이터 수집 전략: **Strava OAuth(자동) + 수동입력(백업)** 2-소스, DB에 `source: 'strava' | 'manual'` 구분.

## 기술 스택
- Next.js 16 App Router · TypeScript · Tailwind CSS v4
- 카드 → PNG: `html-to-image` (브라우저에서 처리, 서버비용 0)
- 정적 export. `npm run build` → `out/` 폴더 그대로 무료 호스팅 업로드.
...
- `app/page.tsx` — 홈(기능 런처). `app/card/page.tsx` — 1단계 생성기.
- `components/RecordCard.tsx` — 캡처 대상 카드 비주얼(스토리 9:16 / 피드 1:1).

## 다음 할 일 (이어서 작업 시)
1. `lib/crew.ts`에 실제 크루 이름/색 반영 후 카드 디자인 톤 맞추기.
2. **2단계 페이스 그룹 편성** 구현: 참가자 목표페이스 입력 → A/B/C조 자동 분반 → 편성표. 백엔드 0, localStorage 정도.
3. 이후 3·4단계에서 Supabase 무료티어 도입.
